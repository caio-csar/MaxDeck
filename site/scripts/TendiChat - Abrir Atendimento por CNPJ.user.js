...
(function () {
    ...
    function extrairContatoDeWamid(id) {
        ...
        const comBrasil = encontrados.find(
            n => n.startsWith('55') && n.length >= 12 && n.length <= 13
        );
    ...
    function extrairContatoTendiChat(article) {
        ...
        const wrappers = Array.from(
            document.querySelectorAll('div[id^="wamid."]')
        );
    ...
    function criarBotao(candidato, article) {
        ...
        btn.dataset.cnpjFormatado =
            cnpjFormatado;

        btn.dataset.valido =
            candidato.valido ? '1' : '0';
    ...
    function criarBotaoSuporteInterno(
        candidato,
        article
    ) {
        ...
        miolo.style.alignItems =
            'center';

        miolo.style.justifyContent =
            'center';
        ...
        miolo.style.borderRadius =
            '50%';

        miolo.style.background =
            '#fff';

        miolo.style.color =
            '#2d8bc5';

        miolo.style.fontSize =
            '13px';

        miolo.style.fontWeight =
            '900';

        miolo.style.fontFamily =
            'Arial, sans-serif';

        miolo.style.lineHeight =
            '1';

        miolo.style.boxSizing =
            'border-box';
    ...
    function processarChatTendiChat() {
        ...
        const artigosDeMensagem =
            document.querySelectorAll(
                'div[id^="wamid."] article'
            );

        artigosDeMensagem.forEach(
            processarMensagem
        );
    }
    ...
    function isPaginaAtendimentoMaxData() {
        ...
        return (
            ...
            /\/Atendimentos\/Atendimento/i.test(
                location.pathname
            )
        );
    }

    function aguardarElemento(
        seletor,
        timeout = 20000
    ) {
    ...
    async function aguardarESelecionarOpcao(
        seletor,
        textoDesejado,
        timeout = 20000
    ) {
        ...
        throw new Error(
            `Opção não encontrada em ${seletor}: ${textoDesejado}`
        );
    }

    async function tentarPreencherInterno(
        descricao,
        acao
    ) {
        ...
        try {
            ...
            console.warn(
                `[TendiChat CNPJ] Não foi possível preencher ${descricao}:`,
                e
            );
    ...
    async function preencherAtendimentoMaxData() {
        ...
        try {
            ...
            console.warn(
                '[TendiChat CNPJ] Falha ao preencher atendimento:',
                e
            );
        }
    }
